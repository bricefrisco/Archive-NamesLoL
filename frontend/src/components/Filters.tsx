...
import {fetchSummoners, getSummonersHaveLoaded, setNameLength} from "../state/namesSlice";
    ...
    return (
        <div className={classes.card}>
            ...
            <FormControl className={classes.control}>
                ...
                <Select displayEmpty labelId='filter-name-length-input' value={nl} onChange={(e) => setNL(e.target.value)}>
                    {menuItems.map(item => <MenuItem key={item} value={item}>{item}</MenuItem>)}
                </Select>
            </FormControl>
            <Button className={classes.button} variant='outlined' color='primary' onClick={apply}>Apply</Button>
        </div>
    )
}
